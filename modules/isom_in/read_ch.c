...
void isor_flush_data(ISOMReader *read, Bool check_buffer_level, Bool is_chunk_flush,int j)
{
	//printf("ord\n");
	u32 i, count;
	GF_Err e = GF_OK;
	...
	//this is a new file, check buffer level
		...
		if (buffer_full) {
			...
			if (count) {
				...
				("[IsoMedia] Buffer level %d ms higher than max allowed %d ms - skipping dispatch\n", com.buffer.occupancy,  com.buffer.max));
			}
			return;
		}
	}

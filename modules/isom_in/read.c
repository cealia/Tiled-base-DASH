...
{
	printf("isor_check_buffer_level\n");
...
GF_Err ISOR_ServiceCommand(GF_InputService *plug, GF_NetworkCommand *com)
{
	...
	if (com->command_type == GF_NET_SERVICE_PROXY_DATA_RECEIVE) {//oh my god
		int j=1;
		printf("jkdshfbhkgvxukdfgvbufdj\n\n\n\n\n\n\n\n\n\n\n");
		isor_flush_data(read, 1, com->proxy_data.is_chunk,7);//<-
		printf("jkdshfbhkgvxukdfgvbufdj\n\n\n\n\n\n\n\n\n\n\n");
		return GF_OK;
	}
	if (com->command_type == GF_NET_SERVICE_FLUSH_DATA) {
		if (read->nb_playing && plug->query_proxy)
			isor_flush_data(read, 0, 0,5);
		return GF_OK;
	}

...
{
	printf("==gf_mpd_parse_adaptation_set==\n");
	u32 i;
	...
	while ( (att = gf_list_enum(root->attributes, &i)) ) {
		if (strstr(att->name, "href")) set->xlink_href = gf_mpd_parse_string(att->value);
			...
			//string test;
			//test = gf_mpd_parse_string(att->value);
			//printf("%s\n",att->name);
			//cout<<"att->name"<<endl;
			set->model = gf_mpd_parse_string(att->value);
			// printf("%s\n",set->model);
			//printf("%d\n",set->min_width);
		} 
	}
...
GF_Err gf_mpd_init_from_dom(GF_XMLNode *root, GF_MPD *mpd, const char *default_base_url)
{
	...
	return gf_mpd_complete_from_dom(root, mpd, default_base_url);//really parse
}

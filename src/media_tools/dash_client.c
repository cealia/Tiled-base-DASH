...
{
	printf("3=====Enter gf_dash_buffer_on=====\n");//YT
	if (!group->dash->enable_buffering) return;
...
(GF_DashClient *dash, GF_DASHFileIOSession *sess, const char *url, u64 start_range, u64 end_range, u32 persistent_mode, GF_DASH_Group *group)
{
	...
	// fout=fopen("log.txt","w+t");

	//  if(fout==NULL) {

	//    printf("Fail To Open File log.txt!!");
	...
	GF_LOG(GF_LOG_DEBUG, GF_LOG_DASH, ("[DASH] Downloading %s starting at UTC "LLU" ms\n", url, gf_net_get_utc() ));
	 fprintf(fout,"[DASH] Downloading %s starting at UTC "LLU" ms\n", url, gf_net_get_utc());//GYT
...
retry:
	...
	case GF_OK:
		...
		fprintf(fout,"[DASH] Download %s complete at UTC "LLU" ms", url, gf_net_get_utc());//GYT
		if(url[39]=='h') fprintf(fout,"<------------\n");
		else fprintf(fout,"\n");
		fprintf(fout,"\n");
		break;
	default:
...
{
	printf("=====Enter gf_dash_set_group_representation=====\n");//YT
#ifndef GPAC_DISABLE_LOG
...
{
	printf("=====Enter gf_dash_switch_group_representation=====\n");//YT
...
static u32 dash_do_rate_adaptation_legacy_rate(GF_DashClient *dash, GF_DASH_Group *group, GF_DASH_Group *base_group,
												...
												GF_MPD_Representation *rep, Bool go_up_bitrate)
{
	printf("@dash_do_rate_adaptation_legacy_rate\n");
	u32 k;
	Bool do_switch;
...
static u32 dash_do_rate_adaptation_legacy_buffer(GF_DashClient *dash, GF_DASH_Group *group, GF_DASH_Group *base_group,
												  ...
												  GF_MPD_Representation *rep, Bool go_up_bitrate)
{
	printf("@dash_do_rate_adaptation_legacy_buffer\n");
	Bool do_switch;
	u32 new_index = group->active_rep_index;
...
static u32 dash_do_rate_adaptation_test(GF_DashClient *dash, GF_DASH_Group *group, GF_DASH_Group *base_group,
												  ...
												  GF_MPD_Representation *rep, Bool go_up_bitrate)
{
	printf("@dash_do_rate_adaptation_test\n");
...
static void dash_do_rate_adaptation(GF_DashClient *dash, GF_DASH_Group *group, int i, int cc)
{
	printf("=====Enter dash_do_rate_adaptation=====\n");
	Double speed;
	...
	printf("i = %d, group->bytes_per_sec = %d\n", i, group->bytes_per_sec);
	...
	*/	
	...
	if (new_index==0xFFFFFFFF) {
		printf("algo return new_index = 0xFFFFFF\n");//YT
	...
	//new_index = 1;
	//if(i==5||i==3||i==1|i==7||i==9) new_index = 1;
	if(i==8) new_index = 2;
	else new_index=1;
	if(i!=0) printf("%s\n",group->adaptation_set->model);
	if (new_index != group->active_rep_index) {
		//new_index = 3;
		printf("new_index != group->active_rep_index\n");
...
{
	printf("4=====Enter gf_dash_download_init_segment=====\n");
	GF_Err e;
	char *base_init_url;
	...
	if (nb_segment_read) {
		...
		dash_do_rate_adaptation(dash, group, 0,0);
	}
...
{
	printf("2=====Enter gf_dash_setup_groups=====\n");//YT
	GF_Err e;
...
{
	printf("1=====Enter gf_dash_setup_period=====\n");//YT
	GF_MPD_Period *period;
...
static DownloadGroupStatus dash_download_group_download(GF_DashClient *dash, GF_DASH_Group *group, GF_DASH_Group *base_group, Bool has_dep_following)
{
	printf("=====Enter dash_download_group_download=====\n");//YT
	...
	//gf_dash_switch_group_representation(dash, group);//YT
...
{
	printf("=====Enter dash_download_group=====\n");//YT
	DownloadGroupStatus res;

	if (!group->current_dep_idx) {
		fprintf(fout,"!group->current_dep_idx\n");
	...
	if (group->groups_depending_on) {
		fprintf(fout,"group->groups_depending_on\n");
...
static void dash_global_rate_adaptation(GF_DashClient *dash, Bool for_postponed_only, int cc)
{
	printf("=====Enter dash_global_rate_adaptation=====\n");//YT
	u32 quality_rank;
	u32 min_bandwidth = 0;
	...
	for (i=0; i<count; i++) {
		...
		if (for_postponed_only) {
			if (group->rate_adaptation_postponed)
				dash_do_rate_adaptation(dash, group, i, cc);
		} else {
			dash_do_rate_adaptation(dash, group, i, cc);
...
{
	printf("=====Enter dash_main_thread_proc=====\n");//YT
	GF_Err e;
...
restart_period:
	...
	while (go_on) {
		...
		/*wait until next segment is needed(new space in buffer)*/
		...
		if (has_postponed_rate_adaptation) {
			dash_global_rate_adaptation(dash, GF_TRUE, 0);
		}
		...
		for (i=0; i<group_count; i++) {
			...
			if (group->selection != GF_DASH_GROUP_SELECTED) {
				fprintf(fout,"group->selection != GF_DASH_GROUP_SELECTED\n");
			...
			if (dash->use_threaded_download) {
				//fprintf(fout,"dash->use_threaded_download\n");
				group->download_th_done = GF_FALSE;
		...
		printf("cc:%d\n",cc++);//YT
		//while(1){}
		dash_global_rate_adaptation(dash, GF_FALSE, cc);
	}
